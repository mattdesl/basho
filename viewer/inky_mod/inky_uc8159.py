...
class Inky:
    ...
    def __init__(self, resolution=(600, 448), colour='multi', cs_pin=CS0_PIN, dc_pin=DC_PIN, reset_pin=RESET_PIN, busy_pin=BUSY_PIN, h_flip=False, v_flip=False, spi_bus=None, i2c_bus=None, gpio=None):  # noqa: E501
        ...
        if resolution not in _RESOLUTION.keys():
            raise ValueError(
                'Resolution {}x{} not supported!'.format(*resolution))
    ...
    def _palette_blend(self, saturation, dtype='uint8'):
        ...
        for i in range(7):
            ...
            rd, gd, bd = [c * (1.0 - saturation)
                          for c in DESATURATED_PALETTE[i]]
            if dtype == 'uint8':
    ...
    def setup(self):
        ...
        if not self._gpio_setup:
            ...
            self._gpio.setup(self.dc_pin, self._gpio.OUT,
                             initial=self._gpio.LOW, pull_up_down=self._gpio.PUD_OFF)
            self._gpio.setup(self.reset_pin, self._gpio.OUT,
                             initial=self._gpio.HIGH, pull_up_down=self._gpio.PUD_OFF)
            self._gpio.setup(self.busy_pin, self._gpio.IN,
    ...
    def _busy_wait(self, timeout=45.0):
    ...
    def set_image_raw(self, image):
        self.buf = image.reshape((self.cols, self.rows))
    ...
    def set_image(self, image, saturation=0.5):
        ...
        if not image.size == (self.width, self.height):
            raise ValueError("Image must be ({}x{}) pixels!".format(
                self.width, self.height))
        if not image.mode == "P":
            if Image is None:
                raise RuntimeError(
                    "PIL is required for converting images: sudo apt install python-pil python3-pil")
